...
def Particles3SCFT2D(intialDensityA,intialDensityB,
                     Height, Depth, Width, BoxHeight, BoxWidth, 
	...
	number_of_iterations 			  = 70000	# Default 30000
	tolerence 						= 10**(-7) 	
	...
	for j in range(0,number_of_lattice_pointsX):
		for i in range(0, number_of_lattice_pointsY):
		      ...
		      if r <= length:
                            PotentialAB[j][i] = ((A1+A2)*(m.cos(3.14159*r/length))/2.0 + (A1 - A2)/2.0)
		      else:
	...
	phic = 1 - intialDensityA - intialDensityB
		...
		for j in xrange(number_of_iterations):
			...
			cnvb  = iphib*Vkab
			cnvc  = iphic*Vkab
			icnva = dx*dy*np.fft.irfft2(cnva)
			icnvb = dx*dy*np.fft.irfft2(cnvb)
			icnvc = dx*dy*np.fft.irfft2(cnvc)
